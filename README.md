...
**LSTM.** A recurrent model reaches 0.993 in cross-validation but only 0.933 on a fresh hold-out, and the gradient saliency maps expose why: the vanilla LSTM keys largely on end-of-cycle frames rather than the biomechanically meaningful mid-stance window. An attention-augmented variant is added to make the model's focus explicit. Averaging its attention weights per condition across the cycle shows that each class is discriminated from a different gait phase: the unbraced model concentrates sharply on the late cycle (around 80%), while the braced conditions spread attention more evenly, giving a per-class, human-readable view of which part of the cycle drives each decision.
...
Export the following plots from the notebook into `figures/` to make this section render. These four tell the story most efficiently for a reader skimming the repository.

| | |
|---|---|
| ![UMAP clusters with Random Forest decision regions](figures/umap_rf_regions.png) | ![Mean attention weight per condition across the gait cycle](figures/lstm_attention_by_condition.png) |
| **UMAP feature space with RF decision regions.** Three clean, well-separated condition clusters. | **LSTM attention by condition.** Mean +/- STD attention across the cycle; the unbraced model peaks sharply in the late cycle (~80%), while braced conditions attend more evenly. |
| ![HMM emission means per hidden state](figures/hmm_emission_means.png) | ![ST-GCN confusion matrix showing the unbraced collapse](figures/stgcn_confusion_matrix.png) |
| **HMM emission means by state.** Phase decomposition of the cycle, with brace-specific modulation of the hip and ankle. | **ST-GCN confusion matrix.** The honest negative result: the unbraced class collapses under limited data. |
