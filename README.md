...
The notebook is the canonical narrative that runs the full analysis and produces the figures, while `gait/` holds the reusable core (preprocessing, the skeleton graph, and the model definitions) as importable modules so the same code can be reused and tested rather than living only inside cells.
