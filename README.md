...
[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/Yogarajan12/gait-bracing-classification/blob/main/multivariate_gait_analysis.ipynb)
...
├── gait/                                  # core reusable components
│   ├── __init__.py
│   ├── preprocessing.py                   # sequence building, smoothing, derivatives
...
The notebook is the canonical narrative that runs the full analysis and produces the figures, while `gait/` holds the reusable core (preprocessing, the skeleton graph, and the model definitions) as importable modules so the same code can be reused and tested rather than living only inside cells. To keep the notebook self-contained and runnable end to end, it retains its own inline definitions; the package mirrors that core as importable, smoke-tested modules for reuse outside the notebook.
...
```bash
git clone https://github.com/Yogarajan12/gait-bracing-classification.git
cd gait-bracing-classification
...
The core components can also be imported directly, for example:

```python
from gait import build_sequences, HMMClassifier, make_wide_residual_stgcn
```

The preprocessing utilities depend only on NumPy and SciPy, while the model builders require the deep-learning and HMM libraries. Run the smoke tests from the repository root with:
...
@misc{gait_bracing_classification,
  ...
  year         = {2025},
  howpublished = {\url{https://github.com/Yogarajan12/gait-bracing-classification}}
}
```
